 ...
 * Version 1.0
 * 
 * This ver adds RGB "neopixel" status LED output
 * @ArcadeMachinist
 * 
 * Version 1.0AM
 *
...
#define RGBLED (PD0)
NeoPixelBus<NeoGrbFeature, NeoSk6812Method> strip(1, (PD0));
...
byte serial[7] = { 0x35, 0x30, 0x30, 0x32, 0x30, 0x35, 0x31  };
...
void setup() {
  ...
  pinModeFast((PD0),OUTPUT);
  
  strip.Begin();
  strip.SetPixelColor(0, RgbColor(105,0,0));
  strip.Show();
 }
...
void select_ext_sync_on() {
      ...
      strip.SetPixelColor(0, RgbColor(0,105,0));
      strip.Show();
}
...
void select_ext_sync_off() {
      digitalWriteFast(EXT_SYNC_OE_n,1);
      digitalWriteFast(BX_OE_n,0);
      strip.SetPixelColor(0, RgbColor(0,16,105));
      strip.Show();
}
...
void deselect() {
      digitalWriteFast(EXT_SYNC_OE_n,1);
      digitalWriteFast(BX_OE_n,1);
      strip.SetPixelColor(0, RgbColor(128,94,0));
      strip.Show();
}
